...
Derived from code originally part of CellProfiler, licensed under both GPL
and BSD licenses.
...
def canny_3d(image, sigma=1., low_threshold=.1, high_threshold=.2, mask=None):
    ...
    * Perform a hysteresis thresholding: first label all points above the
    ...
    if (image.ndim != 2) or (image.ndim != 3):
        raise TypeError("The input 'image' must be a three dimensional array.")
    ...
    sobels = []
    abs_sobels = []
    magnitude = np.zeros_like(sobels)
    for axis in range(image.ndim):
        sobels.append(ndi.sobel(smoothed, axis=axis))
        abs_sobels.append(np.abs(sobels[axis]))
        magnitude += abs_sobels[axis] ** 2
